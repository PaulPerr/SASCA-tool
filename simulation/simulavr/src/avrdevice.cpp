...
unsigned char AvrDevice::GetIOReg(unsigned addr) {
	...
	// With IO get access we MUST do the access only ONCE
	// thus we need to get the value in a tmp variable to use it 2 times after that
	// if this step is not done, than every 2nd char is not handled during the execution
...
bool AvrDevice::SetIOReg(unsigned addr, unsigned char val) {
	assert(addr < ioSpaceSize);  // callers do use 0x00 base, not 0x20
	//leakOnWrite(*(rw[addr + registerSpaceSize]), val);
	// XXX make it a leakOnWrite:
	/* now it will be just a value leakage, waiting for the reply from the mailing list
	 if I try to read the value *(rw[addr + registerSpaceSize]) it is somehow destroyed
	 and the program crushes
	 -> yes indeed hre the read is destructive -> no 'reasonably simple' way of hacking it here
	*/
	leakOnRead(val); 
...
unsigned AvrDevice::GetRegX(void) {
	// R27:R26
	leakOnRead( (*(rw[27]) << 8) + *(rw[26]) );
	return (*(rw[27]) << 8) + *(rw[26]);
}

unsigned AvrDevice::GetRegY(void) {
	// R29:R28
	leakOnRead( (*(rw[29]) << 8) + *(rw[28]));
	return (*(rw[29]) << 8) + *(rw[28]);
}

unsigned AvrDevice::GetRegZ(void) {
	// R31:R30
	leakOnRead( (*(rw[31]) << 8) + *(rw[30]));
	return (*(rw[31]) << 8) + *(rw[30]);
}

// FOR SCA leakage
void AvrDevice::leakOnRead(unsigned char val){
	if (powerTrace_on){
		leakValue(val);
	}
}

void AvrDevice::leakOnRead(int val){
	if (powerTrace_on){
		leakValue(val);
	}
}

void AvrDevice::leakOnWrite(unsigned char oldVal, unsigned char newVal){
	if (powerTrace_on){
		leakTransition(oldVal, newVal);
	}
}
