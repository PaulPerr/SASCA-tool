...
#include <fstream> // SCA powertrace file
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "a:e:f:d:gGm:p:t:P:uxyzhvnisF:R:W:VT:B:c:C:o:l:", long_options, &option_index);
        if(c == -1)
            break;
        ...
        switch(c) {
            ...
            case 'P':
				if(global_verbose_on)
					cout<<"Power trace for SCA activated"<<endl;
				powerTraceFilename = optarg;
				break;
    ...
    // SCA :
	if(powerTraceFilename != ""){
		dev1->powerTrace_on = true;
		//powertrace.open(powerTraceFilename.c_str(), std::ofstream::out | std::ofstream::app);
		//powertrace.open(powerTraceFilename.c_str(), std::ofstream::out| std::ofstream::binary);
               powertrace.open(powerTraceFilename.c_str(), std::ofstream::out);
